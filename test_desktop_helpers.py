...
class DesktopHelperTests(unittest.TestCase):
    ...
    def test_license_activation_reenables_buttons_without_overriding_usb_lock(self):
        ...
        fake.account_is_signed_in = lambda: fake.account_ready
    ...
    def test_customer_app_requires_account_and_active_license(self):
        self.assertFalse(hasattr(locker, "FREE_LOCAL_ACCESS"))
        self.assertIn(
            "open_account_login(required=True)",
            inspect.getsource(locker.USBFileLocker.__init__),
        )
        self.assertIn(
            "not parent.account_is_signed_in()",
            inspect.getsource(locker.ensure_license_feature),
        )
        self.assertIn(
            "license_feature_allowed",
            inspect.getsource(locker.launch_companion_script),
        )
