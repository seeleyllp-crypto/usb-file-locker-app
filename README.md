...
Release `2026.07.16.4` expands Owner Maintenance Operations with a daily briefing, customer-impact state, next-review timing, five-level severity summary, eight scored domain cards, ten fixed aggregate watch metrics, four fixed review windows, eight owner shortcuts, and check-specific deep links. The admin-token-protected web cockpit still runs exactly forty aggregate checks across eight categories and turns failed checks into a prioritized owner runbook.

The current tab can hold a temporary comparison baseline, auto-refresh every sixty seconds, copy a fixed briefing or plan, export a local calendar, print, and create privacy-safe text, JSON, CSV, or browser-generated SHA-256 evidence receipts. No baseline, planner choice, filter, or search state is uploaded. Exports exclude license keys, license IDs, customer identity, owner notes, receipts, machine identifiers, report contents, files, paths, PINs, USB secrets, and customer maintenance history. The console is read-only toward customer PCs and cannot lock, unlock, execute, scan, delete, quarantine, or retrieve local data. API `0.38.0` provides the schema-two protected report. `OWNER OPERATIONS` in License Issuer and Owner Update Lab opens it directly.
