...
Release `2026.07.17.1` expands the customer workspace with a next-best action, four readiness lanes, a seven-day care routine, categorized rank benefits, six guided help paths, privacy guarantees, a scrollable Windows customer view, and a fixed-field customer care-plan export. Checklist progress remains local and the API still receives no customer files, paths, PINs, USB secrets, or machine identity.
