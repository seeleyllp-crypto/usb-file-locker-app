...
Release `2026.07.13.3` expands both the customer Update Center and the private Owner Update Lab. Customers can inspect signed release identity, copy the verified SHA-256, export a privacy-safe verification receipt, run a local installation-readiness check, download a separately verified package copy, review anonymous update activity, open rollback backups, and start an app-data backup before installation. The Owner Update Lab adds a 15-check preflight, signed-package content inspection, report export, hash-chained release history, candidate-folder access, and owner shortcuts. Owner tools remain excluded from customer ZIPs. Signed updates replace app files only and preserve keys, licenses, settings, vault data, audit logs, and locked files.
