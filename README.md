...
Release `2026.07.17.4` adds the Recovery Decision Wizard: seven common customer situations, twenty-one fixed yes-or-no decision points, twenty-eight reviewed outcomes, and 112 ordered action steps. Customers can move back one answer, restart, copy a completed plan, print it, or export fixed public fields. Choices and decision history stay only in the current browser tab.
...
API `0.43.0` serves the fixed decision catalog and continues serving Customer Answers and customer workspace schema 4. It still activates no seat, controls no customer PC, and returns no license proof, customer identity, owner notes, receipts, machine identity, paths, PINs, USB secrets, payment data, or file contents.
