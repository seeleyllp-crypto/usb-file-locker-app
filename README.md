...
Release `2026.07.13.3` adds the customer Vault Health Center for read-only locked-file structure checks, legacy-format review, recovery status, multi-key and key-folder coverage, safe scan cancellation, attention filtering, aggregate snapshot comparison, and an update-preserved local health baseline with automatic drift warnings. Its reports, copied summaries, and baseline contain no filenames, paths, key IDs, secrets, or file contents. It also expands the customer Update Center with signed release identity, verified SHA-256 copy, verification receipts, local readiness checks, verified download-only mode, anonymous update activity, rollback-backup access, and app-data backup. The private Owner Update Lab includes a 15-check preflight, signed-package inspection, report export, hash-chained release history, candidate-folder access, and owner shortcuts. Owner tools remain excluded from customer ZIPs. Signed updates preserve keys, licenses, settings, vault data, audit logs, locked files, and the local health baseline.
