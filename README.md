...
Release `2026.07.18.41` restores required customer authentication and licensing. The Windows app requires a successful VaultLink account sign-in at startup and keeps customer controls disabled until that signed-in account has an active assigned license. Signing out, an expired account session, a revoked or expired license, or an account with no active assignment returns the app to blocked access. Passwords are sent only to the configured HTTPS API, are never saved locally, and the signed account session stays only in memory. The registered owner USB and private Update Lab remain separately protected.
...
Release `2026.07.18.37` adds required in-app customer sign-in. The Windows app asks for the VaultLink account username and password at startup, keeps the signed session only in memory, never saves the password, checks the account every twenty seconds, and automatically activates the license assigned by the owner. Customers no longer paste license keys into the normal License Center flow.
