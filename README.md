...
Release `2026.07.18.14` moves the Local Receipt Review's former bottom action rows, result count, and only `CLOSE` action into a fixed top toolbar. The review window has no page scrollbar or page-scroll behavior; fixed `RESULTS` and `REVIEW DETAILS` tabs keep its content reachable. The main app also removes its sideways scrollbar, with crowded controls arranged in separate normal rows.
