...
Release `2026.07.17.2` adds a five-stage customer journey, anonymous device-seat planning, five support-readiness checks, a four-phase 90-day continuity plan, a current-version digest, a ten-term glossary, and persistent local action progress. The Windows Customer Workspace can mark the next fixed action complete, reset local progress, open the next unfinished action, and include reviewed action IDs in its fixed-field care-plan export. Only fixed action IDs and a UTC timestamp are stored on the Windows account; they are never uploaded.

API `0.41.0` serves customer workspace schema 4. It still activates no seat, controls no customer PC, and returns no license proof, customer identity, owner notes, receipts, machine identity, paths, PINs, USB secrets, payment data, or file contents.
