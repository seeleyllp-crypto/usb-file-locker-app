...
Release `2026.07.17.11` adds a standalone Receipt Inspector inside Download Verification Center. A customer can inspect one explicitly selected VaultLink JSON receipt before selecting any downloaded file. Schema-two integrity seals are verified and fail closed when missing, malformed, or invalid; unsealed schema-one receipts remain readable but are clearly labeled legacy. Comparing a prior receipt also fills its inspection result automatically.
