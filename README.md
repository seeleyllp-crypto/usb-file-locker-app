...
Release `2026.07.16.3` adds Owner Maintenance Operations. The admin-token-protected web cockpit runs exactly forty aggregate checks across eight categories: access and secrets, storage and recovery, signed releases, service and customer surfaces, licensing and seats, support and messaging, audit and incident review, and commerce and governance. Failed checks become a prioritized owner runbook, while release, storage, service, and sixteen customer-surface matrices remain visible in one place.

The operations response and browser-created JSON/CSV exports exclude license keys, license IDs, customer identity, owner notes, receipts, machine identifiers, report contents, files, paths, PINs, USB secrets, and customer maintenance history. The console is read-only toward customer PCs and cannot lock, unlock, execute, scan, delete, quarantine, or retrieve local data. API `0.37.0` provides the protected fixed report. `OWNER OPERATIONS` in License Issuer and Owner Update Lab opens it directly.
