...
Release `2026.07.18.40` adds compact full-release delivery for more reliable automatic updates on slower hosted connections. The signed ZIP contains a pinned SHA-256 bootstrap and one compressed payload with the exact 56-file customer allowlist. Current updaters inspect and install the normal files directly; older supported updaters use the transparent one-time bootstrap. Both paths reject links, unexpected names, duplicates, unsafe paths, oversized content, payload tampering, and incomplete archives. Replaced app files are backed up, failed legacy installs restore the prior desktop entrypoint, temporary payload files are removed, and `%LOCALAPPDATA%\USBFileLocker` customer data is never packaged or replaced.

Release `2026.07.18.39` makes every local customer tool free to use. VaultLink no longer opens a forced sign-in window, disables local buttons, or blocks companion apps when no license is assigned. Customer accounts are optional for online account services, and signing out never disables local locking, unlocking, recovery, privacy, audit viewing, diagnostics, or safety tools. USB-key, optional PIN, owner-USB, and private update-publishing protections are unchanged. Cloud audit uploads still require valid online authorization because they store reports on the API.
...
The app checks the API at startup when its daily update check is due. Optional online plan state has a separate automatic API heartbeat while an account is signed in. Revoked, expired, reset, removed-device, or deactivated online receipts never disable local tools. Update Center verifies an Ed25519 manifest signature and SHA-256 package hash, clearly shows the current release, backs up replaced app files, and preserves everything in `%LOCALAPPDATA%\USBFileLocker`. `AUTO-INSTALL VERIFIED UPDATES` is a visible local opt-in; when enabled, a verified update downloads, verifies again, closes the app, installs, and restarts without an extra prompt. Automatic installation remains disabled inside Git working folders; use `git pull` there.

Release `2026.07.18.38` adds the fixed no-scroll Lock Protection Center and three selectable workflow profiles. `STANDARD` preserves the existing lock flow, `GUARDED` requires an exact one-use Safe Lock Preview approval, and `MAXIMUM` also requires a current metadata-only capacity check, a non-empty PIN, and a successful recovery test for the exact loaded key and current PIN within the last thirty minutes. The selected fixed profile survives updates, while recovery proof, capacity results, queue snapshots, and PIN state stay only in memory and clear when the key unloads, panic lock runs, the key changes, or the app exits.

The bounded capacity check reviews at most 1,000 selected roots and 50,000 nested entries. It reads filesystem metadata to count files, folders, bytes, and storage headroom without reading file contents or changing, locking, deleting, copying, renaming, or uploading anything. Its fixed summary contains aggregate counts and byte totals only; it excludes filenames, paths, drive names, USB key IDs, PINs, secrets, and file contents. Low space, inaccessible items, links or junctions, stale queue selections, scan limits, and canceled checks prevent `MAXIMUM` from starting either lock mode. The Protection Center is available from `MORE TOOLS`, Tool Finder, or `Ctrl+Shift+S`.

Release `2026.07.18.37` added required in-app customer sign-in. Release `2026.07.18.39` supersedes that behavior for local tools: startup sign-in is no longer required, the session remains optional and memory-only, and local tools do not require a license.

Release `2026.07.18.36` adds account-first licensing. Customers must create an account before a new license can be issued. The Windows License Issuer loads the protected API account list, requires an active customer selection, and sends only that account ID with the selected rank and private owner note. New licenses are assigned to the selected account instead of being created from free-form customer labels or email fields.

Release `2026.07.18.35` adds owner-authorized `VERIFIED ROLLBACK SNAPSHOTS`. The Owner Update Lab can download the currently live public release, verify its signed manifest, size, SHA-256, filename, and embedded version, test safe extraction in a temporary folder, and require a Microsoft Defender no-threat result before retaining it.

Each snapshot contains only the public signed ZIP, signed manifest, and bounded metadata under the private owner-lab data folder. Matching snapshots are reverified and reused rather than overwritten. Snapshot creation never installs, restores, publishes, or changes a customer app, and it excludes USB keys, signing keys, licenses, vaults, audit logs, customer records, and personal files.

Release `2026.07.18.34` adds an owner-only `RELEASE DELTA REPORT`. It compares the signed public update ZIP with the latest verified private candidate and reports added, changed, removed, and unchanged packaged app files. It also groups impacted files into Python code, launchers, documentation, configuration, and other files.

The live package must pass its signed-manifest, size, and SHA-256 checks before comparison. The known embedded-version mismatch is shown as blocked rather than hidden. Unsafe paths, links, duplicate names, oversized archives, and private runtime data are rejected. The report shows packaged filenames and counts only, never contents or per-file hashes, and cannot publish or change either repository.

Release `2026.07.18.33` adds an owner-only `LIVE IDENTITY REPORT`. It downloads the bounded public ZIP to a temporary folder and shows the manifest version, embedded app version, package filename agreement, size agreement, SHA-256 agreement, and complete signed-identity result. A failed check includes one bounded validation reason, while a healthy check is reported separately.

The report also compares the latest verified private candidate by version, test count, and whether it is newer than the public manifest. Repair guidance always requires another candidate test and the explicit registered-owner Publish action; the report cannot publish, edit either repository, replace the stable app, or change settings. Its temporary ZIP is deleted automatically, and its history record contains only the reviewed public version and success or failure.

Release `2026.07.18.32` adds strict signed-package identity verification. Every update ZIP must contain exactly one bounded `usb_file_locker.py` entrypoint whose literal `DESKTOP_APP_VERSION` exactly matches the signed manifest and versioned package filename. The compatibility floor cannot be newer than the declared update. Source is parsed as Python syntax only and is never executed during this check.

This prevents an older Owner Update Lab from accidentally packaging newer source under its own older version number. Candidate verification, the installer, and post-publish live verification all enforce the same identity rule. Owner Update Lab now runs a 16th preflight check that downloads the bounded live ZIP to a temporary folder and verifies its signature, filename, size, SHA-256, and embedded version together before reporting release readiness.

Release `2026.07.18.31` adds a faster Tip Center review workflow. `REVIEW PENDING` opens the first remaining built-in tip, four compact category counters show Locking, Recovery, Privacy, and Updates completion, and `UNDO REVIEW` restores the previous reviewed-ID set, focus, and visible tip position. `Ctrl+Enter` starts pending review and `Ctrl+Z` runs the same bounded undo action.

The latest 20 review changes are undoable only during the current app session and start empty after restart. Clearing reviewed marks is also reversible without changing favorites. Fixed-width controls prevent state labels from shifting the layout, and the window remains `720x390` with no canvas, scrollbar, free-form field, or remote content. Only allowlisted reviewed IDs and the selected focus persist.

Release `2026.07.18.30` adds persistent Tip Center review tracking. Customers can mark any built-in tip `REVIEWED` or return it to `TO REVIEW`, see exact progress out of 16, and use the dedicated `TO REVIEW` focus to move through pending guidance only.

Review state is stored only as bounded stable allowlisted tip IDs. Completing the final pending tip safely returns to `ALL`, while `Clear reviewed marks` restores all pending tips without changing favorites. Unknown IDs, duplicates, malformed entries, and excess values are discarded. The fixed `720x390` window still has no canvas, scrollbar, free-form field, or downloaded content.

Release `2026.07.18.29` adds persistent favorites for up to eight built-in Tip Center messages and a dedicated `FAVORITES` focus. Favorites are saved only as stable allowlisted IDs, not copied text. Unknown IDs, duplicates, malformed entries, and items beyond the limit are discarded whenever settings are saved.

The selected fixed focus is also preserved across updates. Selecting Favorites with no saved items is blocked, and removing the last favorite while viewing Favorites safely returns to `ALL`. Navigation, pause state, and copied tip text remain session-only. No filename, path, USB key ID, PIN, password, file content, webhook, or customer record enters favorite settings.

Release `2026.07.18.28` expands Tip Center to 16 built-in messages, split evenly across Locking, Recovery, Privacy, and Updates. A fixed focus menu selects `ALL` or one category; Previous, Next, Reset, and automatic 12-second rotation stay inside that selected set. The position badge shows the category and exact focused position.

`COPY TIP` places only the current built-in message on the Windows clipboard. Focus, position, pause state, and navigation remain in memory for the open app session and are not logged, saved, or uploaded. The fixed window is now `720x390` and still has no canvas, scrollbar, free-form field, webhook, downloaded content, or remote command.

Release `2026.07.18.27` adds a fixed `640x350` Tip Center for the same local guidance shown in the Overview activity strip. Customers can move backward or forward, pause or resume automatic rotation, and reset to the first tip. Manual navigation restarts one 12-second countdown so duplicate timers cannot accumulate.

Tip Center is available from Overview `MORE TOOLS` and Tool Finder. Its position and pause state stay only in the current app session. The window has no canvas, scrollbar, free-form field, webhook, downloaded content, or extra main-page height, and active job progress continues to take priority in Overview.

Release `2026.07.18.26` rotates one of eight fixed local safety and recovery tips in the Overview `CURRENT ACTIVITY` strip every 12 seconds while the app is idle. Active lock, unlock, scan, and verification progress always takes priority, and tips resume after the job finishes.

The messages are built into the signed app. They do not use Discord, webhooks, downloaded text, or free-form commands, and they never read or display filenames, paths, USB key IDs, PINs, passwords, license keys, or file contents. The timer is canceled during shutdown, and the feature adds no popup, panel, scrollbar, or extra page height to the fixed `860x520` layout.

Release `2026.07.18.25` adds Receipt Comparison for any selected session receipt that has an earlier result. The fixed no-scroll comparison shows aggregate status and mode transitions, success-rate trend, requested, successful, failed, unprocessed, verified, removed-original, and duration deltas, plus Preview Guard and canceled-state changes.

Comparison revalidates both receipts, requires different anonymous IDs and chronological order, and never includes filenames, paths, USB key IDs, PINs, secrets, file contents, or error details. Customers can copy a privacy-safe comparison from Receipt History, `MORE TOOLS`, or Tool Finder. New jobs and history clearing close stale comparison windows.

Release `2026.07.18.24` expands Last Lock Receipt into a session-only Receipt History for the latest 10 lock jobs. The fixed no-scroll dashboard shows aggregate session jobs, successful items, success rate, and jobs needing review, while `PREVIOUS` and `NEXT` move between individual receipts.

Customers can copy one privacy-safe receipt or an aggregate session summary and clear the history without changing locked files or audit logs. Every history entry is revalidated and normalized before storage, duplicate receipt IDs are rejected, unknown fields are discarded, and the entire history clears when the USB session unloads or the app exits.

Release `2026.07.18.23` adds a session-only Last Lock Receipt for both `LOCK COPY` and `LOCK + REMOVE ORIGINAL`. After a lock job finishes, the fixed no-scroll receipt window shows an anonymous receipt ID, start and finish times, aggregate requested, successful, failed, and unprocessed counts, whether Preview Guard was used, and the verified and removed-original counts where applicable.

The receipt never stores or displays filenames, paths, USB key IDs, PINs, secrets, file contents, or detailed errors. It is available from the Locker action row, `MORE TOOLS`, and Tool Finder, can be copied as a privacy-safe text summary, updates while open, and clears when the USB session unloads or the app exits.

Release `2026.07.18.22` adds an optional one-time Preview Guard inside `QUEUE TOOLS`. Arm it after a clean Safe Lock Preview to bind the next `LOCK COPY` or `LOCK + REMOVE ORIGINAL` start to the exact in-memory queue and selected targets that were reviewed.

An armed guard blocks the lock if the queue, target selection, 1,000-item bound, USB readiness, or fresh file/folder classification no longer matches. It reports only an aggregate reason, changes no files when blocking, and opens the preview for review. A matching guard is consumed only after the lock job starts; USB unload, panic lock, app exit, or manual clear also removes it. Without an armed guard, existing lock behavior is unchanged.
...
Release `2026.07.18.19` adds `Safe Lock Preview` from `MORE TOOLS`, Tool Finder, or `Ctrl+Shift+P`. It checks up to 1,000 queued entries for files, folders, missing items, duplicates, already-locked entries, links or junctions, unsupported entries, invalid values, inaccessible items, queue truncation, and USB/owner-policy readiness.

The fixed preview dashboard is aggregate-only and has no canvas or scrollbar. It never reads file contents or locks, deletes, renames, copies, or uploads anything. Its copied summary excludes filenames, paths, PINs, key IDs, secrets, and file contents, and it explicitly avoids claiming that a preview proves files are safe.

Release `2026.07.18.18` adds `ALL`, `FAVORITES`, and `RECENT` modes to Tool Finder. Customers can pin up to eight allowlisted tools; only their fixed internal command IDs are stored in local settings. Unknown IDs, duplicates, malformed values, and entries beyond the limit are discarded whenever settings are saved.

Tool Finder also remembers up to eight recently launched tools for the current app session. Recent actions and search text stay in memory only and disappear when the app closes. They are never written to settings, audit logs, exports, or API requests. The expanded finder remains fixed-size with no canvas or scrollbar.

Release `2026.07.18.17` adds a fixed customer `Tool Finder` for 27 allowlisted commands. Open it from `MORE TOOLS` or with `Ctrl+K`, type a local query, and launch the selected existing action. It shows at most ten ranked matches in a fixed `720x460` window with no canvas or scrollbar.

Tool Finder queries remain only in the open window's memory and disappear when it closes. They are never logged, persisted, exported, or sent to the API. Results come only from a fixed internal catalog, so the finder cannot execute free-form text, scripts, file paths, or downloaded commands.

Release `2026.07.18.16` expands the fixed Overview without adding visual clutter or scrolling. `MORE TOOLS` now organizes 23 existing customer destinations into Security & privacy, Recovery, Data & local tools, and Service submenus. A new local readiness check reports aggregate USB access, owner-policy state, queue counts, PIN mode, license state, verified auto-update preference, and current-job state.

The readiness check does not inspect files and is explicitly not a malware scan or security guarantee. It never includes filenames, paths, PIN values, key IDs, license keys, secrets, or file contents.

Release `2026.07.18.15` removes page scrolling from both the main app and Local Receipt Review. The main app uses fixed task tabs, with lock and unlock actions above the file list. Receipt Review moves its former bottom actions into a fixed top toolbar and uses fixed `RESULTS` and `REVIEW DETAILS` tabs. Neither window uses a page canvas, page scrollbar, or page wheel binding.

The new fixed `OVERVIEW` tab shows live local-only access, queue, selection, PIN-mode, license, and current-activity summaries. It includes direct workspace navigation, the most useful safe actions, and a compact `MORE TOOLS` menu. `Copy safe status` exports only aggregate state and explicitly excludes filenames, paths, PIN values, key IDs, secrets, and file contents.
...
Priority state, bulk changes, history, selection, and counts live only in the open review window. They are never logged, exported, persisted, or sent to the API. API `0.58.0` advertises these controls without receiving local review data.

Release `2026.07.18.8` improves large local receipt-review sessions. Filename search now uses a cancellable 180 ms debounce, so rapid typing replaces the previous pending redraw instead of rebuilding up to 1,000 rows on every keystroke. Closing or clearing the review cancels any delayed callback, and a visible numeric row remains selected across search, filter, sort, mark, undo, and reset refreshes when it still matches.

`NEXT PENDING ITEM` now cycles all unreviewed rows in priority order, including Info and Valid results after actionable work. Aggregate progress separately shows reviewed and pending totals plus pending Action Required, Review, Info, and Valid counts. These counts, delayed-callback state, and selection remain local memory only and never contain exported filenames or hashes.

API `0.57.0` advertises the debounce, selection preservation, complete pending navigation, and aggregate level breakdown without receiving filenames, queries, selection, delayed-callback state, progress, local results, selected folders, or aggregate reports.

Release `2026.07.18.7` fixes receipt-review lifecycle and resource-bound bugs. Only one review window can exist, starting another folder audit closes the old review first, and `CLEAR LOCAL LIST` destroys the sole window so no stale filename snapshot remains. Filter rows and review IDs are now consumed with hard bounds instead of materializing an entire supplied iterable.

The review session also gains `UNDO LAST MARK` for up to 100 bounded numeric changes and a clearer aggregate progress line showing reviewed, actionable remaining, and Action Required remaining. Undo history and progress contain only numeric IDs and counts; they never contain filenames or hashes and are never logged, exported, persisted, or sent to the API.

API `0.56.0` advertises these lifecycle, resource-bound, undo, and progress changes without receiving filenames, review IDs, undo history, progress, selected rows, local results, selected folders, or aggregate reports.

Release `2026.07.18.6` adds a memory-only review-session checklist to Local Receipt Review. Customers can mark a visible row Reviewed or Pending, hide reviewed rows, see displayed and completed counts, reset every review mark, and use `NEXT REVIEW ITEM` to skip completed rows while keeping Action Required results first.

Each review mark is represented only by a bounded numeric ID created for the open window. The reviewed set never contains filenames or file hashes, and it disappears when the window closes. Marks, progress, selected rows, searches, filters, sorting, filenames, and local results are never logged, exported, persisted, or sent to the API.

API `0.55.0` advertises the local review-session controls without receiving filenames, review marks, progress, selected rows, navigation, local results, selected folders, or aggregate reports.

Release `2026.07.18.5` adds deterministic customer triage to Local Receipt Review. Every fixed folder-audit result is assigned Action Required, Review, Info, or Valid, with a fixed meaning and safe next step. A new Needs Review filter hides informational and valid-only rows, priority sorting puts failures first, and `NEXT REVIEW ITEM` cycles Action Required rows before other review items.

Triage guidance is fixed app text and never includes a receipt filename. Filenames, selected rows, navigation, guidance state, searches, filters, and sorting remain in memory only and are never logged, exported, persisted, or sent to the API. Triage does not identify malware, rescan a download, or prove that any file is safe.

API `0.54.0` advertises the local triage controls without receiving filenames, selected rows, navigation, guidance state, local results, selected folders, or aggregate reports.

Release `2026.07.18.4` adds local search, fixed result filters, deterministic sorting, and a visible result count to Local Receipt Review. Customers can show all results, problems, valid seals, legacy receipts, skipped entries, or entries not inspected because a limit was reached, then sort by filename or fixed result.

Search text is capped at 80 characters and exists only in the open review window. Search text, filter choices, sorting, filenames, and local results are never added to the aggregate report, its export, audit-log details, API requests, or persisted app state. `CLEAR FILTERS` restores the full list, while `CLEAR LOCAL LIST` removes names from memory without deleting the anonymous aggregate report.

API `0.53.0` advertises these local controls in customer discovery metadata without receiving filenames, search text, filter choices, sorting, local results, selected folders, or aggregate reports.

Release `2026.07.18.3` adds an in-memory Local Receipt Review after a Receipt Folder Audit. The review window identifies each examined top-level entry by sanitized filename and a fixed result such as valid local seal, valid external seal, unsealed legacy, invalid or tampered, skipped link, skipped subfolder, oversized, unreadable, or not inspected because a limit was reached.

Filename details are kept in a separate runtime-only object and never enter the aggregate folder-audit report, its JSON export, audit-log details, API requests, or persisted app state. Display names are capped at 180 characters and control characters are replaced. The review window has no copy or filename-export action, and `CLEAR LOCAL LIST` removes the names from memory while leaving the anonymous aggregate report available.

API `0.52.0` recognizes only fixed actions for opening and clearing the local review. It does not receive receipt filenames, local results, the selected folder, or the aggregate report automatically.

Release `2026.07.18.2` adds a bounded Receipt Folder Audit to Download Verification Center. A customer explicitly chooses one ordinary folder, and the audit considers only top-level JSON files. It never enters subfolders and skips links, junctions, non-JSON entries, and receipts larger than 256 KB. Work is capped at 1,000 top-level entries, 250 JSON candidates, and 32 MB of cumulative receipt data, and it runs on a background thread.

The aggregate result counts valid seals from this profile, valid seals from other profiles, unsealed legacy receipts, invalid or tampered receipts, skipped entries, and reached limits. Each receipt is rejected if it changes while being read. The export excludes the selected folder path, receipt filenames, receipt contents, hashes, signer data, integrity public keys, and validation error text. It does not execute files, upload data, rescan original downloads, or prove that any file is safe.

API `0.51.0` recognizes two fixed audit actions for running and exporting a receipt-folder audit. The selected folder, receipt files, folder audit report, and its counts are not uploaded automatically.

Release `2026.07.18.1` adds a standalone Receipt Inspector inside Download Verification Center. A customer can inspect one explicitly selected VaultLink JSON receipt before selecting any downloaded file. Schema-two integrity seals are verified and fail closed when missing, malformed, or invalid; unsealed schema-one receipts remain readable but are clearly labeled legacy. Comparing a prior receipt also fills its inspection result automatically.

The copyable inspection report includes only the receipt SHA-256 and fixed categories for integrity, coarse size, extension class, signature state, Defender state, detected type, extension/header state, PE architecture, warning IDs, and bounded aggregate archive information. Imported paths, filenames, unknown fields, signer text, signer fingerprints, integrity public keys, archive entry names, and file contents are discarded. Arbitrary imported size, extension, type, or architecture text is converted to a fixed safe category rather than displayed.

API `0.50.0` recognizes two additional fixed audit action names for receipt inspection and copying its safe summary. The selected receipt, its path, its contents, its public key, its SHA-256, and the inspection report are not uploaded automatically.

Release `2026.07.17.10` adds local Ed25519 integrity seals to exported Download Verification Center receipts. The private signing key is generated locally, protected with Windows DPAPI for the current Windows user, and stored under preserved `%LOCALAPPDATA%\USBFileLocker` data. The private key is never written into a receipt; only its random public key, key fingerprint, and signature are exported.

Sealed schema-two receipts fail closed when their seal is missing, malformed, or invalid. Imports label a valid seal from this Windows profile, a valid seal from another profile, or an unsealed schema-one legacy receipt. Removing a seal cannot silently look valid: it either fails as a damaged schema-two receipt or becomes visibly unsealed legacy data after an explicit schema downgrade. A seal detects edits after export but is not a public code-signing certificate and does not prove that the checked file is safe. The stable random public key can also show that multiple shared receipts used the same local signing key.

API `0.49.0` publishes the local receipt-integrity capability. Receipts, public or private receipt keys, files, hashes, paths, filenames, signer details, and comparison output are not uploaded automatically.
