...
Release `2026.07.13.3` expands the private Owner Update Lab with a 15-check preflight, signed-package content inspection, SHA-256 copy, privacy-safe JSON report export, hash-chained release history, candidate-folder access, and shortcuts to the live owner pages and pinned GitHub repositories. The owner tools remain excluded from customer ZIPs. The signed update still replaces app files only and preserves keys, licenses, settings, vault data, audit logs, and locked files.
